...
- Der Client soll den aktuellen Chat-Titel als `chatName` übergeben; alternativ akzeptiert das Tool die Aliasfelder `chatTitle`, `conversationTitle` und `threadTitle`.
- Wenn kein Chat-Titel übergeben wird, nutzt das Tool den Fallback `Current Chat` und gibt dafür einen Warning-Hinweis zurück.
